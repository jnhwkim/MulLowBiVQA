...
require 'Dropout'
--Multimodal Low-rank Bilinear Attention Networks (MLBdo)
--Use 1x1 convolution for dimension reduction
netdef = {}
function netdef.MLBdo(rnn_size_q,nhimage,common_embedding_size,joint_dropout,num_layers,noutput,batch_size,glimpse)
   ...
   for i=1,glimpse do
      local visual_embedding_=nn.Sequential()
            ...
            :add(nn.ParallelTable()
               ...
               :add(nn.SplitTable(2)))
            :add(nn.MixtureTable())
            :add(nn.Dropout(p,false,false,false,false,true,nn.Sequential()
...
function netdef.MLBdo_updateBatchSize(net,nhimage,common_embedding_size,num_layers,batch_size,glimpse)
   local ngrid=14*14
