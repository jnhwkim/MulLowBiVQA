...
function netdef.MLBdc(rnn_size_q,nhimage,common_embedding_size,joint_dropout,num_layers,noutput,batch_size,glimpse)
   ...
   for i=1,glimpse do
      local visual_embedding_=nn.Sequential()
            ...
            -- :add(nn.Dropout(p))
            :add(nn.Identity())
            :add(nn.LinearDropconnect(nhimage, common_embedding_size, 0.5, 'Tanh'))
            :add(nn.Identity())
   ...
   for i=1,num_layers do
      ...
      local attention=nn.Sequential()  -- attention networks
            :add(nn.ParallelTable()
               :add(nn.Sequential()
                  -- :add(nn.Dropout(p))
                  :add(nn.Identity())
                  :add(nn.LinearDropconnect(rnn_size_q, common_embedding_size, 0.5, 'Tanh'))
                  :add(nn.Identity())
      ...
      multimodal_net:add(nn.ParallelTable()
         ...
         :add(nn.Sequential()
            ...
            -- :add(nn.Dropout(p))
            :add(nn.Identity())
            :add(nn.LinearDropconnect(rnn_size_q, common_embedding_size*glimpse, 0.5, 'Tanh'))
            :add(nn.Identity()))
