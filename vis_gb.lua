...
if opt.rnn_model == 'GRU' then
   ...
   bgru = nn.GRU(embedding_size_q, rnn_size_q)
   bgru:maskZero(1)
   -- bgru:trimZero(1)
   --encoder: RNN body
...
paths.mkdir('visualize_gb')
f = io.open('visualize_gb/generate.txt', 'w')
function visualize(model, s, e, scores, x)
   ...
   for h5_idx=s,e do  -- for each sample
      ...
      image.save('visualize_gb/'..target_ques_id..'_im0.png', im0)  -- original
      image.save('visualize_gb/'..target_ques_id..'_sm0.png', sm0)  -- softmax0
      image.save('visualize_gb/'..target_ques_id..'_sm1.png', sm1)  -- softmax1
      ...
      for i=1,3 do
        out[i]:copy((out[i] - torch.mean(out)) / torch.std(out))
      end
      
      image.save('visualize_gb/'..target_ques_id..'_vgd.png', out+0.5)  -- grounding V
